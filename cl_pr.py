...
# Instructions for classification (unchanged)
...
# Improved batch processing function for main category
def classify_main_category_batch(crime_infos, model, tokenizer, instruction):
    messages_list = []
    ...
    inputs = tokenizer(messages_list, return_tensors="pt", padding=True, truncation=True).to("cuda")
...
# Improved batch processing function for subcategory
def classify_subcategory_batch(crime_infos, model, tokenizer, instruction):
    messages_list = []
    for crime_info in crime_infos:
        messages = [{
            "role": "user",
...
def process_batch(batch_of_crime_infos):
    # Get main categories for the batch
    categories = classify_main_category_batch(batch_of_crime_infos, main_model, main_tokenizer, MAIN_INSTRUCTION)
    ...
    for idx, (crime_info, category) in enumerate(zip(batch_of_crime_infos, categories)):
    ...
    results = [None] * len(batch_of_crime_infos)
    
    # Process each category if there are any entries
    if financial_crimes:
        financial_subcategories = classify_subcategory_batch(financial_crimes, finance_model, finance_tokenizer, FINANCE_INSTRUCTION)
        for i, idx in enumerate(financial_indices):
            results[idx] = {"category": "financial_crimes", "subcategory": financial_subcategories[i]}
    
    if women_children_crimes:
        women_subcategories = classify_subcategory_batch(women_children_crimes, women_model, women_tokenizer, WOMEN_INSTRUCTION)
        for i, idx in enumerate(women_indices):
            results[idx] = {"category": "crime_against_women_and_children", "subcategory": women_subcategories[i]}
    
    if cyber_attack_crimes:
        cyber_subcategories = classify_subcategory_batch(cyber_attack_crimes, cyber_attack_model, cyber_attack_tokenizer, CYBER_ATTACK_INSTRUCTION)
        for i, idx in enumerate(cyber_indices):
            results[idx] = {"category": "cyber_attack_or_dependent_crimes", "subcategory": cyber_subcategories[i]}
    
    if other_crimes:
        other_subcategories = classify_subcategory_batch(other_crimes, other_model, other_tokenizer, OTHER_INSTRUCTION)
        for i, idx in enumerate(other_indices):
            results[idx] = {"category": "other_cyber_crime", "subcategory": other_subcategories[i]}
    
    return results

# Main function to process the entire dataset
def process_dataset(csv_file, output_file, batch_size=BATCH_SIZE):
    ...
    # Convert results to DataFrame and add to original DataFrame
    results_df = pd.DataFrame(all_results)
    df['category'] = results_df['category']
    df['subcategory'] = results_df['subcategory']
...
if __name__ == "__main__":
    ...
    output_file = 'updated_crimes_parallel.csv'
